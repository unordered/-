// profiler1009.cpp : 정적 라이브러리를 위한 함수를 정의합니다.
//
...
void ProfileDataOutText(WCHAR* szFileName)
{
	for (int index = 0; index < 50; index++)
	{
		...
		{
			wprintf(L"tag:%s, time: %lld us\n", profile_list[index].tag_name, (profile_list[index].sumTime.QuadPart / 10 ) / profile_list->tick);
		}
	}
